...
class fake_hd:
    ...
    def __init__(self,wrms,omega_m=0.3,w=-1.,MB=-19.2):
    ...
    def plot_contour(self):
        plt.figure()
        self.cx,self.cy,self.grid=self.conf_interval('om','w',80)
...
if __name__=='__main__':
    ...
    fhh = fake_hd(0.15,omega_m=0.3,w=-1.,MB=-19.2)
    fhh.hubble_fit()
    cx, cy, cz = fhh.conf_interval('om','w', 60)

    fh = fake_hd(0.13,omega_m=0.3,w=-1.,MB=-19.2)
    fh.hubble_fit()
    cxx, cyy, czz = fh.conf_interval('om','w', 60)

    plt.figure()
    ylim = plt.ylim(-1.4,-0.8)
    xlim = plt.xlim(0.2,0.40)
    ...
    plt.legend([p1, p2], ['SALT2 like', 'SUGAR like'])
