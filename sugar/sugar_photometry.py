...
def plot_filter_PF(WRITE=False):
    dic=cPickle.load(open('/sps/snovae/user/leget/CABALLO/SUGAR_validation/all_CABALLO_data_binning_speed_with_SI.pkl'))
    Y=dic['PTF09dnl']['spectra']['Y_flux']
    X=dic['PTF09dnl']['spectra']['X']

    fig,ax1=P.subplots(figsize=(16,8))
    P.subplots_adjust(left=0.07, right=0.93,bottom=0.07,top=0.97)
    ...
    ax2.set_ylim(0,1.2)
    ax2.set_ylabel('Filters transmission',fontsize=16)
    ...
    ax2.legend([p1, p2, p3, p4, p5], ['$U_{PFL}$', '$B_{PFL}$','$V_{PFL}$','$R_{PFL}$','$I_{PFL}$'],loc=4)
...
class extract_SNf_light_curves:

    def __init__(self,dic_photo):

        self.dic=cPickle.load(open(dic_photo))

    def select_sn_data(self,sn_name):
        
        self.BVR=[]
        self.BVR_err=[]
        
        self.BVR.append(self.dic[sn_name]['photometry']['BSNf'])
        self.BVR.append(self.dic[sn_name]['photometry']['VSNf'])
        self.BVR.append(self.dic[sn_name]['photometry']['RSNf'])
        self.BVR.append(self.dic[sn_name]['photometry']['USNf'])
        self.BVR.append(self.dic[sn_name]['photometry']['ISNf'])
        self.BVR.append(self.dic[sn_name]['photometry']['B'])

        #self.BVR_err.append(self.dic[sn_name]['photometry']['BSNf_err'])
        #self.BVR_err.append(self.dic[sn_name]['photometry']['VSNf_err'])
        #self.BVR_err.append(self.dic[sn_name]['photometry']['RSNf_err'])

        self.BVR=N.array(self.BVR)
        self.BVR_err=N.array(self.BVR_err)

        self.Time=self.dic[sn_name]['photometry']['Time']
        self.redshift=self.dic[sn_name]['spectra']['0']['z_cmb']
...
if __name__=='__main__':
    ...
    #plot_filter_PF(WRITE=True)


    #Make_movie('/sps/snovae/user/leget/CABALLO/SUGAR_validation/SUGAR_model_v1.asci','/sps/snovae/user/leget/CABALLO/Prediction_GP_binning_speed_without_MFR_issue/hyperparameters.dat',Q='1',Var=3)

    #print 'pouet'
    #SP=cPickle.load(open('/sps/snovae/user/leget/CABALLO/SUGAR_validation/SUGAR_parameters_without_cosmology.pkl'))
    #sn_name=SP.keys()
    #TEMPS=N.linspace(-12,42,35)
    #SN_true=extract_SNf_light_curves('/sps/snovae/user/leget/CABALLO/SUGAR_validation/all_CABALLO_data_with_BD17_photometry_SNf_filter.pkl')
    #SN_model=MC_light_curve('/sps/snovae/user/leget/CABALLO/SUGAR_validation/SUGAR_model_v1.asci','/sps/snovae/user/leget/CABALLO/Prediction_GP_binning_speed_without_MFR_issue/hyperparameters.dat',TEMPS)
    ###META=cPickle.load(open('/sps/snovae/user/leget/CABALLO/META_JLA.pkl'))
    ...
    #sn='PTF09dnl'
        ###print sn
    #SN_true.select_sn_data(sn)
    #SN_model_on_data=MC_light_curve('/sps/snovae/user/leget/CABALLO/SUGAR_validation/SUGAR_model_v1.asci','/sps/snovae/user/leget/CABALLO/Prediction_GP_binning_speed_without_MFR_issue/hyperparameters.dat',SN_true.Time)
    #SN_model.build_fixed_light_curves(N.array([SP[sn]['x1'],SP[sn]['x2'],SP[sn]['x3']]),SP[sn]['Av'],SP[sn]['Grey'])
    #SN_model_on_data.build_fixed_light_curves(N.array([SP[sn]['x1'],SP[sn]['x2'],SP[sn]['x3']]),SP[sn]['Av'],SP[sn]['Grey'])
    #rms,NMAD=plot_light_curve(sn,SN_true.BVR,SN_true.Time,SN_model.BVR,SN_model.Time,SN_model_on_data.BVR)    

        ###P.savefig('../These_plot/plot_phd/Chapitre9/SUGAR_light_curves/'+sn+'_training.pdf')
        ###P.close()
        ###RMS.append(rms)
        ###nmad_sugar.append(NMAD)
        ###nmad_salt2.append([META[sn]['salt2.BSNf.dfit.nmad'],META[sn]['salt2.VSNf.dfit.nmad'],META[sn]['salt2.RSNf.dfit.nmad']])

    ###C=['b','g','r']
    ###FILTRESNF=['$B_{SNf}$','$V_{SNf}$','$R_{SNf}$']
    ###nmad_sugar=N.array(nmad_sugar)
    ###nmad_salt2=N.array(nmad_salt2)
    ###for i in range(3):
        ###P.figure()
        ###P.hist(nmad_sugar[:,i],histtype='stepfilled',alpha=1,color=C[i],label='SUGAR, mean nMAD=%.4f'%(N.mean(nmad_sugar[:,i])))
        ###P.hist(nmad_salt2[:,i],histtype='stepfilled',alpha=0.4,color=C[i],label='SALT2.4, mean nMAD=%.4f'%(N.mean(nmad_salt2[:,i])))
        ###P.ylim(0,35)
        ###P.xlabel(FILTRESNF[i]+' nMAD')
        ###P.ylabel('number of supernovae')
        ###P.legend()
        #P.savefig('../These_plot/plot_mars_2016/nMAD_filter_%i_training.pdf'%(i))

    
    #SF=SUGAR_parameter_photometry(test.BVR,test.BVR_err,test.Time,test.redshift,SUGAR_model='/sps/snovae/user/leget/CABALLO/SUGAR_model.asci')
    #SF.comp_chi2(0,0,0,0,0)
    #SF.fit_SUGAR_parameter()
    #SF.plot_result()
